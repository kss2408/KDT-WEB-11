...
const app = express();
const PORT = 8000;
let sign = ' '; //전역변수로 두고 password 저장 (db)라고 생각.
...
app.post('/sign', (req, res) => {
    const { pw } = req.body;
    // const sign = createHashedPassword(pw);   //순서대로 주석처리하며 쓰는중
    // sign = createPbkdf(pw);
...
const createHashedPassword = (password) => {
    //createHashe(알고리즘).update(암호화할값).digest(인코딩방식)
    return crypto.createHash('sha256').updata(password).digest('base64');
};

//pdkdf2함수
const salt = crypto.randomBytes(16).toString('base64'); //솔트생성
const iterations = 1000; //반복횟수
const keylen = 64; //생성한 키의 길이
...
//양방향
const algorithm = 'aes-256-cbc'; //256비트키를 사용하고 블록크기는 128비트
const key = crypto.randomBytes(32);
const iv = crypto.randomBytes(16); // 초기화벡터, 보완성을 높이기 위해 사용
//암호화
...
const decipher = (encryptedData) => {
    ...
    let decryptedData = decipher.update(encryptedData, 'base64', 'utf-8'); // 위와 역순으로 쓰면 된다.
